	...
	switch(type){
		...
		case 2:{
					if(subtype == 0){
						...
						Mix_VolumeChunk(enemy_dead, volumen-20);
					}
		...
		case 4:{
					if(subtype == 0){
						...
						Mix_VolumeChunk(bullet1, volumen);
					}
					if(subtype == 1){
						Mix_PlayChannel(3,bullet2, loops);
						Mix_VolumeChunk(bullet2, volumen);
					}
					if(subtype == 2){
						Mix_PlayChannel(3,bullet1, loops);
						Mix_VolumeChunk(bullet1, volumen);
					...
					if(subtype == 3){
						...
						Mix_VolumeChunk(bullet3, volumen);
					}
