...
bool ClientParser::loadConfiguration() {
	...
	this->tiXmlFileConfig = new TiXmlDocument ("Client/data/configurationClient.xml");
	if (this->tiXmlFileConfig->LoadFile()) {
		this->fileConfigLoaded = true;
		evaluateTags();
	} else {
		this->tiXmlDefaultFileConfig = new TiXmlDocument ("Client/data/configurationClientDefault.xml");
